...
pub fn poll_event(mut seq: &[u8]) -> Option<Event> {
    ...
    match seq {
        ...
        [b] if b.is_ascii_graphic() || *b == b' ' => Some(Event::Keyboard(KeyEvent::Char(*b as char))),
        ...
        seq if seq.starts_with(b"\x1b[M") || seq.starts_with(b"\x1b[<") => Some(Event::Mouse(parse_mouse_event(seq)?)),
        _ => None
    }
}
    ...
    match seq {
        ...
        seq if seq.starts_with(b"\x1b[<") => {
            ...
            match last_char {
                ...
                'm' => Some(MouseEvent::Release(MouseData { button: btn, pos: MousePos{x: cx, y: cy} })),
                _ => None
            }
        },
        // X10 Mode
        seq if seq.starts_with(b"\x1b[M") && seq.len() == 6 => {
            ...
            if is_motion {
                return Some(MouseEvent::Move(MouseData { button: button, pos: MousePos { x: cx, y: cy } }));
            } else if is_pressed{
                return Some(MouseEvent::Pressed(MouseData { button: button, pos: MousePos{x: cx, y: cy } }));
            } else if !is_pressed && !is_motion && button == MouseButton::None {
                return Some(MouseEvent::Release(MouseData { button: MouseButton::None, pos: MousePos{x : cx, y: cy } }));
            } else {
                return None;
            }
        },
